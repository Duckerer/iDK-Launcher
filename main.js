...
const { safeSeg, isSafeUrl, ipcHandler } = require('./src/common/utils');
...
function withInstallLock(fn) {
  const run = installQueue.then(async () => {
    if (installActive) throw new Error(tl('toast.anotherInstall'));
    installActive = true;
    try {
      return await fn();
    } finally {
...
function setupVersionsWatcher() {
  if (versionsWatcher) {
    try { versionsWatcher.close(); } catch {}
  ...
  try {
    ...
    versionsWatcher.on('error', () => {
      try { versionsWatcher && versionsWatcher.close(); } catch {}
      versionsWatcher = null;
    });
  } catch {}
}
...
function registerIpc() {
  ...
  ipcMain.handle('dir:open', ipcHandler(async (sub) => {
  ...
  ipcMain.handle('versions:list', async () => {
    let manifest = null;
    try { manifest = await versions.getManifest(); } catch {}
    const installed = versions.installedVersions().reduce((m, v) => { m[v.id] = v; return m; }, {});
    const seen = new Set((manifest && manifest.versions || []).map((v) => v.id));
    const extra = versions.installedVersions()
  ...
  ipcMain.handle('versions:delete', ipcHandler(async (id) => {
  ...
  ipcMain.handle('versions:install', ipcHandler(async (id, url) => {
    if (!safeSeg(id)) return { ok: false, error: tl('main.badPath') };
  ...
  ipcMain.handle('auth:offline', ipcHandler((nick) => ({ ok: true, account: auth.loginOffline(nick) })));
  ipcMain.handle('auth:elyby', ipcHandler(async (username, password) => ({ ok: true, account: await auth.loginElyby(username, password) })));
  ipcMain.handle('auth:microsoft', ipcHandler(async () => {
    const account = await auth.loginMicrosoft({ status: (s) => send('auth:status', s) });
    return { ok: true, account };
  }));
  ipcMain.handle('auth:select', (_e, uuid) => { auth.selectAccount(uuid); return true; });
  ipcMain.handle('auth:remove', (_e, uuid) => { auth.removeAccount(uuid); return true; });

  ipcMain.handle('game:launch', ipcHandler(async (payload) => {
    ...
    if (account.type === 'elyby') {
      try { authlibInjector = await authlib.ensure(); } catch (e) {
    ...
    launch.launchGame({
      ...
      onExit: (code) => send('game:exit', { code, versionId, modpack }),
    });
  ...
  ipcMain.handle('game:openLogs', ipcHandler(async (payload) => {
    const dir = path.join(runGameDir(payload), 'logs');
    const err = await shell.openPath(dir);
    return err ? { ok: false, error: err } : { ok: true };
  }));

  ipcMain.handle('news:get', ipcHandler(async (force) => ({
    ok: true, channel: news.CHANNEL, items: await news.fetchNews(!!force),
  })));

  ipcMain.handle('modloaders:loaders', ipcHandler(async (mcVersion) => ({
    ok: true, loaders: await modloaders.listLoadersForVersion(mcVersion),
  })));
  ipcMain.handle('modloaders:install', ipcHandler(async (payload) => {
    return withInstallLock(async () => {
  ...
  ipcMain.handle('shell:open', ipcHandler(async (url) => {
    if (!url || !isSafeUrl(url)) return { ok: false, error: 'Invalid URL' };
    await shell.openExternal(url);
    return { ok: true };
  }));

  ipcMain.handle('mods:search', ipcHandler(async (payload) => {
    const r = await mods.search(payload);
    return { ok: true, mods: r.list, total: r.total };
  }));
  ipcMain.handle('mods:install', ipcHandler(async (payload) => {
    return withInstallLock(async () => ({ ok: true, result: await mods.install(payload) }));
  }));
  ipcMain.handle('mods:installed', () => mods.installedList());
  ipcMain.handle('mods:versions', ipcHandler(async (id) => ({ ok: true, result: await mods.versions(id) })));
  ipcMain.handle('mods:game-versions', ipcHandler(async () => ({ ok: true, versions: await mods.gameVersionsForPicker() })));

  ipcMain.handle('modpacks:search', ipcHandler(async (payload) => {
    const r = await modpacks.search(payload);
    return { ok: true, mods: r.list, total: r.total };
  }));
  ipcMain.handle('modpacks:versions', ipcHandler(async (id) => ({
    ok: true, versions: await modpacks.versionsForPicker(id),
  })));
  ipcMain.handle('modpacks:install', ipcHandler(async (payload) => {
    return withInstallLock(async () => {
  ...
  ipcMain.handle('modpacks:create', ipcHandler(async (payload) => {
    return withInstallLock(async () => {
  ...
  ipcMain.handle('modpacks:list', ipcHandler(() => ({ ok: true, packs: modpacks.list() })));
  ipcMain.handle('modpacks:remove', ipcHandler(async (slug) => {
    if (!safeSeg(slug)) return { ok: false, error: tl('main.badPath') };
    modpacks.remove(slug);
    return { ok: true };
  }));

  ipcMain.handle('skins:get', ipcHandler(async (uuid) => await skins.profile(uuid)));
  ipcMain.handle('skins:setDefault', ipcHandler(async (uuid, style) => await skins.setDefault(uuid, style)));
  ipcMain.handle('skins:upload', ipcHandler(async (uuid, file, slim) => await skins.upload(uuid, file, slim)));
  ipcMain.handle('skins:applyB64', ipcHandler(async (uuid, b64, slim) => await skins.applyB64(uuid, b64, slim)));
  ipcMain.handle('skins:reset', ipcHandler(async (uuid) => await skins.reset(uuid)));
  ipcMain.handle('skins:pick', ipcHandler(async () => await skins.pick()));
  ipcMain.handle('skins:elybyLogin', ipcHandler(async (uuid) => {
